...
> ✅ 已完成：项目列表多选、右侧多选提示、批量删除/导出/已读 MCP 操作均已接入。
...
> ✅ 已完成：项目模型输出 `application/x-llmcabinet-project-ids`，标签树接收标签/标签前缀节点 drop，主窗口通过 `Repository.batch_add_tag()` 批量追加标签。

3. **撤销支持**（nice-to-have，P2）：
   - 拖放打标签后，状态栏短暂显示"已为 N 个项目添加标签「X」— 撤销"
   - 点击"撤销"调 `_set_tags` 移除刚加的标签

> 未做：撤销入口属于 nice-to-have，留给后续打磨。
