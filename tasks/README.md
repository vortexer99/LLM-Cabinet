...
| [19-field-type-change-safety.md](./19-field-type-change-safety.md) | 字段类型变更的安全护栏：库设置弹窗确认 + supersede pending + 字段助手 type_conflict 改为「批准=原地改 / 驳回=不动」 | S+S | P1 | Phase A ✅ 2026-06-02 · Phase B 待做 |
| [20-unify-field-storage.md](./20-unify-field-storage.md) | 废弃系统字段的 projects 列分流，所有非保护字段值统一存 `project_field_values`（📦 schema v3 → v4） | M | P2 | 待做（卡片挂起；建议 #19 Phase B 后再动） |
