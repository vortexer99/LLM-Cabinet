...
**工作量**：M+L（拆为 T0~T4，可分批落地）
**优先级**：T0 = P0（短期已落地）, T1/T2 = P1，T3/T4 = P2
**状态**：T0 ✅ 2026-06-01 · T1/T2/T3/T4 远期
...
本任务拆为 5 个子任务，按价值递减排列：

| 子任务 | 内容 | 优先级 | 工作量 |
|---|---|---|---|
| **T0** | **短期补丁**（已完成）：在 `app/llm/context.py` 内**直接**为 `.pptx`/`.odt`/`.odp`/`.ods`/`.epub`/`.html`/`.rtf` 加纯标准库解析器；新增 `extraction_capability(path)` 公开 API 给 UI 用；LLM 元数据建议对话框文件列表加「内容提取」列展示能否提取 | P0 | XS |
...
T0 解决了"调 LLM 时 .pptx/.epub 等结构化文档全是占位"的尴尬，
但每次调用仍要现场解析；T1+ 才会把摘要持久化到文件级属性，让多消费方共享。
