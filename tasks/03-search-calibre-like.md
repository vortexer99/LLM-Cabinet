...
- [x] **搜索历史下拉**：焦点进入搜索框时，自动弹出最近搜索列表（存储在 `settings` 表 `key='search_history'`，JSON 数组，最多 20 条）。点击历史条目直接填入并触发搜索。菜单提供删除历史入口。
- [x] **收藏表达式**：搜索框右侧加 ⭐ 按钮。当前搜索内容非空时点击弹出命名对话框 → 存入 `settings` 表 `key='saved_searches'`，JSON 格式 `[{"name": "高分科幻", "query": "tag:科幻 AND rating:>=4"}, ...]`。收藏在下拉列表顶部用 ⭐ 前缀区分，菜单提供删除收藏入口。
...
- Phase C：
  - [x] 搜索历史：执行搜索后自动保存，再次点开搜索框显示最近 5 条
  - [x] 收藏表达式：保存"高分科幻"对应 `tag:科幻 AND rating:>=4`，下次从下拉选中直接执行
