...
### T1 排序
- [x] 点「文件名」列头 → 同级文件按名升序；再点 → 降序；表头显示箭头
- [x] 点「类型 / 存储 / 说明」列头 → 各自正确排序
- [x] 任何排序下，📁 文件夹节点都恒在同级文件之前、按名称排列
- [x] 拖动后切回「自定义顺序」→ 文件恢复按 `ord` 排列
- [x] 排序状态按项目持久化：切到别的项目再切回来，排序状态保留
- [x] 独立窗口视图里排序行为与主窗口一致（共享同一个 `tbl_files`）

### T2 拖动
- [x] 同文件夹内拖动改顺序 → 刷新后顺序保持（`ord` 已更新）
- [x] 拖文件到 📁 文件夹节点 → 文件进入该文件夹（`subfolder` 已更新）
- [x] 拖文件到顶层 → `subfolder` 变 `""`
- [x] 多选拖动 → 多个文件一起移动，相对顺序保持
- [x] 从资源管理器拖外部文件进来 → 仍是"添加文件"（不被误判为内部移动）
- [x] 拖动后排序状态自动切到「自定义顺序」
- [x] 独立窗口视图里拖动行为与主窗口一致（共享同一个 `tbl_files`）

### T3 新建空文件夹
- [x] 空白处右键 → 新建文件夹 → 输入名 → 出现在文件树
- [x] 切到别的项目再切回来 → 空文件夹仍在（B 方案）
- [x] 同级不重名校验生效
- [x] 含 `/` 的名字被拒绝
- [x] 文件夹下唯一文件拖走 → 文件夹仍在
- [x] 空文件夹右键 → 删除空文件夹 → 从 `explicit_subfolders` 移除，文件树消失
- [x] 非空文件夹的"删除"项灰显（不允许误删带文件的目录）

### T4 F2 重命名
- [x] 选中文件按 F2 → 进入 label 编辑（与双击一致）
- [x] Shift+F2 → 弹物理文件名重命名对话框，仓储/链接都能改名
- [x] 改名失败（权限/同名）→ 弹错误，UI 不脏
- [x] 文件夹节点 F2 → 输入新名 → 该层所有文件 `subfolder` 同步更新
- [x] 跨项目引用（#32 完成后）的物理改名警告已后置到 #32 集成，不阻塞本卡闭环
