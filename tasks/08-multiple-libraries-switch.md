...
label 编辑后写回 `cabinet.json` 中对应条目，立即在主菜单"最近打开"中体现。
...
1. 用户点击「切换库 → ...选目录」
2. 校验目录合法性：
   - 已是有效库（含 `.llm-cabinet` 标记 / `cabinet.db`）→ 直接当库打开（顺便校验 `user_version <= SCHEMA_VERSION`，若大于则警告）
   - **任何其它情况**（空目录 / 普通目录 / 含其它文件的目录）→ 报错并明确引导：
     "目录不是有效的 LLM Cabinet 库；如果想在此创建新库，请改用「库 → 新建库...」走完整向导"。
     **不在此处顺手创建库** — 创建必须经过 task #15 的多页向导（要采集库描述 / 字段 / 视图模式 /
     API 迁移等），否则会绕过 onboarding 留下半残的库
...
| 最近列表右键菜单（移除/删除/改名）+ 当前库信息对话框可编辑 label | 0.4 天 |
| 设置页改造 | 0.25 天 |
| 异常路径处理 + 老用户迁移 | 0.25 天 |
