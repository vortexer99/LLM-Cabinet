...
- **Search and tags**: use the top search box to filter project titles/descriptions, combine it with sidebar tag filters or tag-folder filters, and drag selected projects onto sidebar tags to assign them in bulk. Tags are a first-class multi-value field.
...
- **File tree view**: files are organized in a collapsible tree by logical subfolder (`files.subfolder` in DB), decoupled from physical storage. Directories appear as 📁 nodes; tree view supports per-project column sorting, internal drag-and-drop reordering/moving, persistent empty folders, and F2/Shift+F2 rename flows. Empty folders dropped in create 0-file projects; deep/large imports trigger a confirmation dialog.
