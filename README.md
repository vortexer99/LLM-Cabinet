...
- **Search and tags**: use the top search box for Calibre-like queries such as `tag:科幻 AND author:刘慈欣 AND rating:>=4`; plain keywords search project titles, descriptions, tags, custom field values, and file names/notes, while field filters support `AND` / `OR` / `NOT` and parentheses. Successful searches are saved to recent history, and useful expressions can be starred for reuse. Sidebar tag filters or tag-folder filters combine with the query as `AND` by default, with an optional whole-library search toggle; selected projects can be dragged onto sidebar tags for bulk assignment. Tags are a first-class multi-value field.
