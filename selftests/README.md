...
| [gui_main_window_regressions.py](./gui_main_window_regressions.py) | 主窗口 GUI 回归：搜索菜单 FocusIn 防重复、全库搜索 toggle、GUI 宽关键词命中字段/文件、网格/列表右键 MCP 已读目标、MCP 未读筛选刷新、主 splitter 宽度保存/读取与右栏宽度稳定；需 PySide6 + offscreen Qt | 跨任务 GUI 回归 |
