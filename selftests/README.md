...
| [gui_main_window_regressions.py](./gui_main_window_regressions.py) | 主窗口 GUI 回归：搜索菜单 FocusIn 防重复、全库搜索 toggle、GUI 宽关键词命中字段/文件、网格/列表右键 MCP 已读目标、MCP 未读筛选刷新、目录/ZIP 项目包导出导入 round-trip、主 splitter 宽度保存/读取与右栏宽度稳定；需 PySide6 + offscreen Qt | 跨任务 GUI 回归 |
| [gui_sample_library_regressions.py](./gui_sample_library_regressions.py) | 样例库 GUI 回归：生成临时 sample library 后覆盖基线项目/字段、搜索/筛选、文件树/扁平视图/来源过滤、项目编辑 LLM 建议、MCP audit 筛选与同名外链导出唯一性；需 PySide6 + offscreen Qt | 样例库手测自动化 |
