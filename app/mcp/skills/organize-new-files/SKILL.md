...
description: "将文件导入 LLM Cabinet 项目库——扫描目录、匹配已有项目或创建新项目、自动填写标题/描述/标签/自定义字段值。使用场景：'帮我把这些论文入库'、'整理下载的文件'、'导入这个文件夹'、'这个 PDF 加到库里'、'把 D 盘 papers 目录下的文件都入库'。"
---
...
> **如果用户明确要求"创建新项目"或"每个文件单独建项目"，直接跳到第 5 步。**

对每个文件依次执行：

1. **提取搜索关键词**：从文件中提取有意义的部分。去掉扩展名、数字编号、日期前缀等噪音。
...
从待处理的文件中取出一个作为当前文件，执行 5a-5e。完成后取出下一个文件，回到 5a，直到所有待处理文件都入库完毕。
...
当前项目的文件入库后，扫描剩余的待处理文件列表，找出与当前项目**高度相关**的文件（如属于同一篇论文的多个附件、同一个软件的不同版本），将其一并添加到该项目中，并从待处理列表中移除。判断标准：

- 文件名/目录名包含相同论文标题或项目名称
- 文件类型互补（如 `.pdf` + `.tex` + `.bib` 属于同一论文）
- 目录中相邻且无明显独立标识的区别文件

如果不确定是否相关，不合并，让用户单独决策。
---
