...
## v0.6.0 发布验证（2026-09-09）

- 本地编译检查和 28 个自测脚本全部通过，共 1190 条断言；含离屏 GUI 回归。
- 备份使用移除配置 API Key 的数据库快照，排除 SQLite 日志及旧迁移备份；恢复后需重填密钥，同机也一样。用户文件保持原样。
- 离屏测试不替代真实屏幕的字体、布局验收；发布 exe 仍需单独验证启动。
